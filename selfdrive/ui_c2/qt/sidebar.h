...
class Sidebar : public QFrame {
  ...
  Q_PROPERTY(ItemStatus diskStatus MEMBER disk_status NOTIFY valueChanged);
...
protected:
  ...
  const QColor good_color = QColor(0, 153, 102);
  const QColor warning_color = QColor(218, 202, 37);
  const QColor danger_color = QColor(201, 34, 49);

  ItemStatus connect_status, panda_status, temp_status,cpu_status, gpu_status,mem_status, disk_status;
  QString net_type;
  int net_strength = 0;
