...
void ui_update_params(UIState *s) {
  ...
  // UI显示设置
  s->scene.dp_show_date_time = params.getBool("dp_show_date_time");
  s->scene.dp_hide_ui = params.getBool("dp_hide_ui");
  s->scene.dp_lon_acm = params.getBool("dp_lon_acm");

  // 调参参数
  s->scene.dp_torque_lat_accel_factor = params.get("dp_torque_lat_accel_factor");
  s->scene.dp_torque_friction = params.get("dp_torque_friction");
  s->scene.dp_lateral_torque_kp = params.get("dp_lateral_torque_kp");
  s->scene.dp_lateral_torque_ki = params.get("dp_lateral_torque_ki");
  s->scene.dp_long_pid_kp = params.get("dp_long_pid_kp");
  s->scene.dp_long_pid_ki = params.get("dp_long_pid_ki");
...
void UIState::updateStatus() {
  ...
  // 参数更新：每 200 帧更新一次（约 10秒）
  static int param_update_timer = 0;
  param_update_timer++;

  if (param_update_timer % 200 == 0) {
...
UIState::UIState(QObject *parent) : QObject(parent) {
  sm = std::make_unique<SubMaster, const std::initializer_list<const char *>>({
    ...
    "gpsLocationExternal",
    // dp -for ui
    "liveParameters","liveTorqueParameters",
  });
