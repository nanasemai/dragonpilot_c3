...
class TestALKALatActive(unittest.TestCase):
  ...
  This mirrors the logic in controlsd.py:
    ...
    alka_active = alka_enabled and CS.cruiseState.available and not standstill and calibrated and \
                  not CS.seatbeltUnlatched and not CS.doorOpen
  ...
  def _compute_alka_active(self, alka_enabled, cruise_available, standstill, calibrated, seatbelt_unlatched, door_open):
    """Compute alka_active matching controlsd.py logic."""
    return alka_enabled and cruise_available and not standstill and calibrated and \
  ...
  def test_lat_active_normal_mode(self):
    ...
    alka_active = self._compute_alka_active(
      alka_enabled=False, cruise_available=True, standstill=False,
  ...
  def test_lat_active_alka_mode(self):
    ...
    alka_active = self._compute_alka_active(
      alka_enabled=True, cruise_available=True, standstill=False,
      calibrated=True, seatbelt_unlatched=False, door_open=False)
    lat_active = self._compute_lat_active(
      selfdrive_active=False, alka_active=alka_active,
      steer_fault_temp=False, steer_fault_perm=False, standstill=False)
  ...
  def test_lat_active_alka_requires_cruise_available(self):
    """ALKA requires cruiseState.available (ACC Main ON)."""
    alka_active = self._compute_alka_active(
      alka_enabled=True, cruise_available=False, standstill=False,
      calibrated=True, seatbelt_unlatched=False, door_open=False)
    lat_active = self._compute_lat_active(
      selfdrive_active=False, alka_active=alka_active,
      steer_fault_temp=False, steer_fault_perm=False, standstill=False)
  ...
  def test_lat_active_alka_requires_moving(self):
    """ALKA requires vehicle to be moving (not standstill)."""
    alka_active = self._compute_alka_active(
      alka_enabled=True, cruise_available=True, standstill=True,
      calibrated=True, seatbelt_unlatched=False, door_open=False)
    lat_active = self._compute_lat_active(
      selfdrive_active=False, alka_active=alka_active,
...
class TestALKAAllConditions(unittest.TestCase):
  ...
  def test_alka_all_conditions_truth_table(self):
    ...
    # Test cases: (alka_enabled, cruise_avail, standstill, calibrated, seatbelt_unlatched, door_open) -> expected_alka_active
    test_cases = [
      # All conditions met
      (True, True, False, True, False, False, True),
      # Missing one condition each
      (False, True, False, True, False, False, False),  # ALKA disabled
      (True, False, False, True, False, False, False),  # No cruise
      (True, True, True, True, False, False, False),    # Standstill
      (True, True, False, False, False, False, False),  # Not calibrated
      (True, True, False, True, True, False, False),    # Seatbelt unlatched
      (True, True, False, True, False, True, False),    # Door open
      # Multiple conditions missing
      (True, False, True, True, False, False, False),   # No cruise + standstill
      (True, True, False, False, True, True, False),    # Not calibrated + seatbelt + door
    ]

    for alka_enabled, cruise_avail, standstill, calibrated, seatbelt_unlatched, door_open, expected in test_cases:
      alka_active = alka_enabled and cruise_avail and not standstill and calibrated and \
                    not seatbelt_unlatched and not door_open

      self.assertEqual(alka_active, expected,
                       f"Failed for alka_enabled={alka_enabled}, cruise_avail={cruise_avail}, "
                       f"standstill={standstill}, calibrated={calibrated}, "
                       f"seatbelt_unlatched={seatbelt_unlatched}, door_open={door_open}")
