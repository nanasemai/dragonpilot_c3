...
static void hyundai_rx_ext_hook(const CANPacket_t *msg) {
  if (alka_allowed && ((alternative_experience & ALT_EXP_ALKA) != 0)) {
    // Track LKAS button (BCM_PO_11 0x391) - only on bus 0
    if ((msg->addr == 0x391U) && (msg->bus == 0U)) {
      hyundai_lkas_button_check(GET_BIT(msg, 4U));
    }

    // Track ACC main state (SCC11 0x420, bit 0 = MainMode_ACC)
    if ((msg->addr == 0x420U) && (msg->bus == (hyundai_camera_scc ? 2U : 0U)) && !hyundai_longitudinal) {
      hyundai_acc_main_check(GET_BIT(msg, 0U));
    }
  }
}
