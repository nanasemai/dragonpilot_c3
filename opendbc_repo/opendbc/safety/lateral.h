...
// - steering_disengage(bool): driver override disables ALKA (e.g., hands on wheel)
static bool lat_control_allowed(void) {
  return controls_allowed || (alka_allowed && (alternative_experience & ALT_EXP_ALKA) != 0 && lkas_on && vehicle_moving && !steering_disengage);
}
