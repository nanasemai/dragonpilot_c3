...
void ignition_can_hook(CANPacket_t *to_push) {
  ...
  if (bus == 0) {
    ...
    if ((addr == 0x1F1) && (len == 8)) {
      ...
      ignition_can = (GET_BYTES(to_push, 0, 1) & 0x2U) != 0U;
      ignition_can_cnt = 0U;
    }
    ...
    if ((addr == 0x152) && (len == 8)) {
      ...
      int counter = GET_BYTES(to_push, 1, 1) & 0xFU;  // max is only 14
      ...
      if ((counter == ((prev_counter_rivian + 1) % 15)) && (prev_counter_rivian != -1)) {
        ...
        ignition_can = ((GET_BYTES(to_push, 7, 1) >> 4U) & 0x3U) == 1U;  // VDM_EpasPowerMode_Drive_On=1
        ignition_can_cnt = 0U;
      }
    ...
    // Mazda exception
    if ((addr == 0x9E) && (len == 8)) {
      ignition_can = (GET_BYTES(to_push, 0, 1) >> 5) == 0x6U;
      ignition_can_cnt = 0U;
    }
